...
var options = {
  ...
  sass: {
    // exclude styles.scss & style-guide-only directory
    // which are only used for styleguide
    minFiles: 'dist/**/*.css',
    files: 'sass/**/*.scss',
    destination: 'dist'
  },
  ...
  styleGuide: {
    ...
    css: [
      path.relative(paths.styleGuide, paths.styles.styleguide + 'styles.css'),
      path.relative(paths.styleGuide, paths.styles.styleguide + 'style-guide-only/kss-only.css')
    ],
    js: [],
...
//require('./gulp-tasks/compile-styleguide-css')(gulp, plugins, options);
...
require('./gulp-tasks/minify-svg')(gulp, plugins, options);
